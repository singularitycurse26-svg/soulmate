...
# Use 30 minutes = 1800s. With 18s clips, that's 100 clips.
# At ~3 min per clip (including rate limit wait), that's ~5 hours.
# Let's use a shorter version first - 3 minutes (180s) = 10 clips to prove it works
DURATION = 180  # 3 minutes first, then we can scale up
...
print(f"Expected clips: {DURATION // 18}")
print(f"Expected time: ~{(DURATION // 18) * 4} minutes (with rate limiting)")
...
sftp = c.open_sftp()
with sftp.open("/tmp/intime2_v2_payload.json", "w") as f:
    f.write(payload)
sftp.close()

print("\n=== Submitting In Time 2 (real motion video) ===")
_, stdout, _ = c.exec_command(
    "curl -s -X POST http://localhost:8546/v1/soulmovies/create "
    "-H 'Content-Type: application/json' "
    "-d @/tmp/intime2_v2_payload.json"
)
...
last_progress = -1
last_status = ""
for i in range(300):  # up to 5 hours
    time.sleep(60)
    ...
    try:
        ...
        if progress != last_progress or status_str != last_status:
            elapsed = i * 60
            mins = elapsed // 60
            print(f"  [{mins}m] status={status_str} progress={progress:.2f}")
...
if final.get('output_path'):
    ...
    _, stdout, _ = c.exec_command(f"ffprobe -v quiet -show_entries stream=codec_name,width,height,duration -of csv {final['output_path']} 2>&1")
    print(f"Video: {stdout.read().decode()}")
    print(f"\nDownload: https://191.44.121.29.sslip.io/v1/soulmovies/download/{pid}")

# Tier logs
_, stdout, _ = c.exec_command("grep -i 'tier 0.*\\|agnes.*\\|scene.*\\|succeeded' /tmp/api_server.log | tail -30")
print(f"\n=== Agnes logs ===\n{stdout.read().decode()}")
