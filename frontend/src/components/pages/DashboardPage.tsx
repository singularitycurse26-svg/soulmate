...
export function DashboardPage() {
  ...
  const [showWakkiiSocial, setShowWakkiiSocial] = useState(false);
  ...
  const avatarColor = (name?: string) => {
    const colors = ["#6366f1", "#8b5cf6", "#06b6d4", "#f59e0b", "#ec4899", "#22c55e", "#38bdf8", "#a855f7"];
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="w-8 h-8 border-4 rounded-full animate-spin" style={{ borderColor: `${OS_ACCENT} transparent transparent transparent` }} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      {/* Top Bar */}
      <div className="sticky top-0 z-40 flex items-center justify-between px-4 h-14 rounded-2xl surface">
        <div className="flex items-center gap-2 flex-1">
          <button onClick={() => setActivePage("dashboard")} className="text-lg font-semibold tracking-tight">Feed</button>
          <div className="relative hidden sm:block">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 rounded-full text-sm bg-bg-alt border-border w-64"
            />
            {showSearch && searchResults.length > 0 && (
              <div className="absolute top-full mt-1 left-0 w-full surface py-2 max-h-80 overflow-y-auto z-50">
                {searchResults.map((user) => (
                  <button key={user.id} onClick={() => { setShowSearch(false); setSearchQuery(""); setSearchResults([]); showAlert("info", `User: ${user.name}`); }} className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-left">
        ...
        <div className="flex items-center gap-2 flex-1 justify-end">
          <div className="relative">
            <button
              ...
              className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center relative"
            >
          ...
          <button onClick={() => setActivePage("contacts")} className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center">
            <MessageCircle className="w-5 h-5" />
          </button>

          <button onClick={() => setActivePage("security")} className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center font-bold text-sm overflow-hidden">
      ...
      <div className="flex max-w-[1920px] mx-auto">
        ...
        <aside className="hidden md:flex flex-col w-[360px] flex-shrink-0 p-4 gap-1 sticky top-14 h-[calc(100vh-56px)] overflow-y-auto">
          <button className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          <button onClick={() => setActivePage("dating")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          <button onClick={() => setActivePage("marketplace")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          <button onClick={() => setActivePage("contacts")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          <button onClick={() => setShowWakkiiSocial(!showWakkiiSocial)} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
            <div className="w-9 h-9 rounded-full bg-pink-100 flex items-center justify-center"><Radio className="w-5 h-5 text-pink-500" /></div>
            <span className="font-medium text-sm">Wakkii Social</span>
            {showWakkiiSocial && <ChevronUp className="w-4 h-4 ml-auto text-muted" />}
          </button>
          <button onClick={() => setActivePage("dashboard")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          <button onClick={() => setActivePage("wallet")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          <button onClick={() => setSeeMore(!seeMore)} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
          ...
          {seeMore && (
            <>
              <button onClick={() => setActivePage("ai")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
              ...
              <button onClick={() => setActivePage("games")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
              ...
              <button onClick={() => setActivePage("incentives")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
              ...
              <button onClick={() => setActivePage("healing")} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
        ...
        <main className="flex-1 max-w-[680px] mx-auto p-4 space-y-4">
          ...
          <div className="card">
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowCreatePost(true)} className="flex-1 text-left px-4 py-2.5 rounded-full bg-bg-alt hover:bg-white/5 text-muted text-sm">
                What's on your mind, {profile?.name?.split(" ")[0] || "User"}?
              </button>
            </div>
            <div className="border-t border-white/5 mt-3 pt-3 flex items-center justify-around">
              <button onClick={() => setShowCreatePost(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-white/5">
                <Video className="w-5 h-5 text-red-400" /><span className="text-sm font-medium text-muted">Live Video</span>
              </button>
              <button onClick={() => setShowCreatePost(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-white/5">
                <ImageIcon className="w-5 h-5 text-green-400" /><span className="text-sm font-medium text-muted">Photo/Video</span>
              </button>
              <button onClick={() => setShowCreatePost(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-white/5">
                <Smile className="w-5 h-5 text-yellow-400" /><span className="text-sm font-medium text-muted">Feeling</span>
              </button>
            </div>
          </div>
          ...
          {posts.length === 0 ? (
            ...
            <div key={post.id} className="card overflow-hidden p-0">
              <div className="flex items-center gap-3 p-4 pb-2">
                ...
                <button className="p-2 rounded-full hover:bg-white/5"><MoreHorizontal className="w-5 h-5 text-muted" /></button>
              </div>
          ...
          {showWakkiiSocial && (
            <div className="card p-4 space-y-4">
              <div className="flex items-center gap-2 pb-3 border-b border-white/5">
                ...
                <h3 className="font-bold text-base">Wakkii Social</h3>
                <span className="text-xs text-muted ml-auto">voice rooms · social · radio · blind date · AI</span>
              </div>
              <WakkiiLinks
                userName={authEmail || "Soulmate User"}
                mode="ptt"
                defaultRole="speaker"
                embedded
              />
        ...
        <aside className="hidden xl:flex flex-col w-[300px] flex-shrink-0 p-4 gap-3 sticky top-14 h-[calc(100vh-56px)] overflow-y-auto">
          <div>
            ...
            <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 cursor-pointer" onClick={() => setActivePage("wallet")}>
          ...
          <div className="border-t border-gray-300 pt-3">
            ...
            <div className="space-y-1">
              {friends.length === 0 ? <p className="text-xs text-gray-400">No friends yet</p> : friends.map((friend) => (
                <button key={friend.id} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 text-left">
                  <div className="relative">
      ...
      {showCreatePost && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] px-4" onClick={() => setShowCreatePost(false)}>
          <div className="surface w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-white/5">
              <h3 className="font-semibold text-lg">Create Post</h3>
              <button onClick={() => setShowCreatePost(false)} className="w-8 h-8 rounded-full bg-bg-alt hover:bg-white/5 flex items-center justify-center"><X className="w-5 h-5 text-muted" /></button>
            </div>
            <div className="p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-semibold text-sm">{profile?.name || authEmail || "User"}</p>
                  <div className="flex items-center gap-1 text-xs text-muted bg-bg-alt px-2 py-0.5 rounded">Public <ChevronDown className="w-3 h-3" /></div>
                </div>
              </div>
              <textarea value={postText} onChange={(e) => setPostText(e.target.value)} placeholder={`What's on your mind, ${profile?.name?.split(" ")[0] || "User"}?`} className="w-full text-lg outline-none resize-none h-32 bg-transparent" autoFocus />
              {postImage && (
              ...
              {showVideoInput && (
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    autoFocus
                  />
                  <button onClick={() => setShowVideoInput(false)} className="px-3 py-2 rounded-lg bg-gray-100 hover:bg-white/5 text-sm text-gray-700">Done</button>
                </div>
              )}
              ...
              <button onClick={handleCreatePost} disabled={!postText.trim() && !postImage.trim() && !postVideo.trim()} className="w-full mt-3 py-2.5 rounded-lg font-bold text-white disabled:opacity-40 bg-accent">Post</button>
            </div>
          </div>
        </div>
      )}
