#!/usr/bin/env node
/**
 * SSH patch: Add social, marketplace, dating routes + fix CORS + auto-wallet to api_server.py on the VPS.
 * Uses ssh2 package for SSH/SFTP.
 */
...
const SSH_USER = "root";
const SSH_PASSWORD = "wallmartxxxxxxxx8";
const REMOTE_FILE = "/opt/incentives-wallet/api_server.py";
const PATCHED_MARKER = "SOCIAL_MARKETPLACE_DATING_PATCHED";
...
def _get_user_id(request):
    try:
        token = request.headers.get("x-session-token", "") if request else ""
        parts = token.split("_")
...
async def create_post(request: Request):
    ...
    db = _social_db()
    c = db.cursor()
    c.execute("INSERT INTO posts (user_id, text, image_url, privacy) VALUES (?, ?, ?, ?)", (uid, body.get("text",""), body.get("image_url"), body.get("privacy","public")))
    db.commit()
    pid = c.lastrowid
    db.close()
...
async def get_post(post_id: int):
    db = _social_db()
    r = db.execute("SELECT * FROM posts WHERE id=?", (post_id,)).fetchone()
    db.close()
...
async def update_profile(request: Request):
    ...
    uid = _get_user_id(request)
    db = _social_db()
    db.execute("INSERT OR REPLACE INTO profiles (user_id, bio, avatar, cover) VALUES (?, ?, ?, ?)", (uid, body.get("bio",""), body.get("avatar",""), body.get("cover","")))
    db.commit()
    db.close()
    return {"ok": True}
...
async def get_listings(category: str = "", min_price: str = "", max_price: str = "", currency: str = "", search: str = "", sort: str = ""):
    ...
    if search: q += " AND (title LIKE ? OR description LIKE ?)"; params += ["%"+search+"%", "%"+search+"%"]
...
async def edit_listing(listing_id: int, request: Request):
    ...
    uid = _get_user_id(request)
    db = _social_db()
    db.execute("UPDATE listings SET title=?, description=?, price=? WHERE id=? AND user_id=?", (body.get("title",""), body.get("description",""), body.get("price",""), listing_id, uid))
    db.commit()
    db.close()
    return {"ok": True}
...
async def update_dating_profile(request: Request):
    ...
    uid = _get_user_id(request)
    db = _social_db()
    db.execute("UPDATE dating_profiles SET bio=?, interests=? WHERE user_id=?", (body.get("bio",""), ",".join(body.get("interests",[])), uid))
    db.commit()
    db.close()
    return {"ok": True}
...
async def dating_suggestions():
    ...
    if swiped_ids:
        ...
        rows = db.execute("SELECT * FROM dating_profiles WHERE user_id != ? AND user_id NOT IN ("+placeholders+")", [uid]+swiped_ids).fetchall()
    else:
  ...
  conn.sftp((err, sftp) => {
    ...
    readStream.on("end", () => {
      try {
        ...
        // Fix CORS — replace allow_origins list with wildcard
        content = content.replace(/allow_origins=\[[^\]]*\]/g, 'allow_origins=["*"]');
        // Fix allow_methods to include all methods
        content = content.replace(/allow_methods=\["GET",\s*"POST"\]/g, 'allow_methods=["GET", "POST", "PUT", "DELETE", "OPTIONS"]');

        // Insert before static files marker, or before catch-all route, or append
        const marker = "from fastapi.staticfiles import StaticFiles";
        const catchall = '@app.get("/{path:path}")';
        if (content.includes(marker)) {
          content = content.replace(marker, INSERT_CODE + "\n" + marker);
        } else if (content.includes(catchall)) {
          content = content.replace(catchall, INSERT_CODE + "\n" + catchall);
        } else {
          content += INSERT_CODE;
        }
        ...
        writeStream.on("close", () => {
          console.log("Patched api_server.py with social + marketplace + dating + auto-wallet + CORS");
          restartAndVerify();
        });
...
function restartAndVerify() {
  ...
  conn.exec("pkill -f api_server 2>/dev/null; sleep 2; systemctl restart incentives-wallet 2>&1; sleep 5", (err, stream) => {
    if (err) { console.error("Exec error:", err); conn.end(); return; }
    let out = "";
    stream.on("data", (d) => { out += d.toString(); });
    stream.on("close", () => {
      console.log("Restart output:", out.trim().slice(0, 200));

      // Verify endpoints
      const checks = [
        "curl -s http://localhost:8546/v1/health 2>&1",
        "curl -s http://localhost:8546/v1/social/feed 2>&1",
        "curl -s http://localhost:8546/v1/marketplace/listings 2>&1",
        "curl -s http://localhost:8546/v1/dating/suggestions 2>&1",
      ];
      let done = 0;
      checks.forEach((cmd, i) => {
        conn.exec(cmd, (err2, s2) => {
          if (err2) { console.error("Check error:", err2); return; }
          let o = "";
          s2.on("data", (d) => { o += d.toString(); });
          s2.on("close", () => {
            const labels = ["Health", "Social feed", "Marketplace", "Dating"];
            console.log(`${labels[i]}: ${o.trim().slice(0, 200)}`);
            done++;
...
conn.connect({ host: SSH_HOST, username: SSH_USER, password: SSH_PASSWORD, readyTimeout: 15000 });
